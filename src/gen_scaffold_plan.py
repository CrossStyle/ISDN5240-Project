...
#     """
#     Create a rotation matrix from euler angles (xyz sequence).
...
#
#
# def compute_fromto(pos, half_length, euler=[0,0,0]):
#     """
#     Compute fromto for a cylinder given center pos, half_length, and euler.
#     """
#     dir_vec = np.array([0, 0, 1])  # Default axis along z
#     R = rotation_matrix_from_euler(euler)
#     dir_vec = R @ dir_vec
#     from_point = pos - half_length * dir_vec
#     to_point = pos + half_length * dir_vec
#     return from_point, to_point
#
#
# def generate_scaffold_xml():
#     """
#     Algorithm for generating scaffold based on facade geoms.
#     1. Extract contour from facade geoms (hardcoded for this facade: flat middle, curved ends approximated by stepped sections).
#     2. Define y sections for middle (symmetric), and mirrored for transition and outer.
#     3. For each section, generate vertical, horizontal x/y, and diagonals if needed.
#     4. Use fromto for all geoms to comply with schema.
#     5. Add third layer to make vertical poles taller (up to 3.2).
#     """
#     tube_radius = 0.024
#     material = "metal_mat"
#     rgba = "0.7 0.7 0.7 1"
#     num_layers = 3
#     vertical_half_lengths = [0.6, 0.5, 0.5]
#     horizontal_levels = [0.2, 1.2, 2.2, 3.2]
#     layer_z_low = [0.2, 1.2, 2.2]
#     layer_z_high = [1.2, 2.2, 3.2]
#     z_centers = [0.6, 1.7, 2.7]
#
#     # Middle section (symmetric, no mirror)
#     middle_section = {"y_start": -0.5, "y_end": 0.5, "add_diagonal": True, "x_front": 2.4, "x_back": 3.0}
#
#     # Positive sections (mirror for negative)
#     positive_sections = [
#         {"y_start": 0.5, "y_end": 1.1, "add_diagonal": False, "x_front": 2.4, "x_back": 3.0},
#         {"y_start": 1.1, "y_end": 2.1, "add_diagonal": True, "x_front": 1.8, "x_back": 2.4},
#     ]
#
#     # All sections: middle + positive + negative
#     sections = [middle_section]
#     for sec in positive_sections:
#         sections.append(sec)
#         neg_sec = sec.copy()
#         neg_sec["y_start"] = -sec["y_end"]
#         neg_sec["y_end"] = -sec["y_start"]
#         sections.append(neg_sec)
#
#     # Start XML
#     xml = '<?xml version="1.0" ?>\n<mujoco>\n'
...
#     xml += '    <asset>\n'
#     xml += '        <texture type="2d" name="groundplane" builtin="checker" mark="edge" rgb1="0.2 0.3 0.4" rgb2 ="0.1 0.2 0.3" markrgb ="0.8 0.8 0.8" width="300" height ="300"/>\n'
...
#     xml += '        <geom name="wall" type="box" size="3 0.025 3" pos="3.2 0 0" euler="0 0 1.57" rgba="0.8 0.6 0.4 1" material="wood_mat"/>\n'
#     xml += '        <geom name="wall2" type="cylinder" size="0.5 1.5" pos="3.2 -1.8 1.5" euler="0 0 1.57" rgba="0.8 0.6 0.4 1" material="wood_mat"/>\n'
#     xml += '        <geom name="wall3" type="cylinder" size="0.5 1.5" pos="3.2 1.8 1.5" euler="0 0 1.57" rgba="0.8 0.6 0.4 1" material="wood_mat"/>\n'
#
#     tube_id = 0
#
#     for sec in sections:
...
#
#         # Vertical poles at y_start and y_end, front and back, for each layer
#         for layer in range(num_layers):
#             half_length = vertical_half_lengths[layer]
#             z_center = z_centers[layer]
#             # Front y_start
#             from_p = [x_front, y_start, z_center - half_length]
#             to_p = [x_front, y_start, z_center + half_length]
...
#             tube_id += 1
#             # Front y_end
#             from_p = [x_front, y_end, z_center - half_length]
#             to_p = [x_front, y_end, z_center + half_length]
#             xml += f'        <geom name="tube{tube_id}" type="cylinder" fromto="{from_p[0]:.1f} {from_p[1]:.1f} {from_p[2]:.1f} {to_p[0]:.1f} {to_p[1]:.1f} {to_p[2]:.1f}" size="{tube_radius}" rgba="{rgba}" material="{material}"/>\n'
#             tube_id += 1
#             # Back y_start
#             from_p = [x_back, y_start, z_center - half_length]
#             to_p = [x_back, y_start, z_center + half_length]
#             xml += f'        <geom name="tube{tube_id}" type="cylinder" fromto="{from_p[0]:.1f} {from_p[1]:.1f} {from_p[2]:.1f} {to_p[0]:.1f} {to_p[1]:.1f} {to_p[2]:.1f}" size="{tube_radius}" rgba="{rgba}" material="{material}"/>\n'
#             tube_id += 1
#             # Back y_end
#             from_p = [x_back, y_end, z_center - half_length]
#             to_p = [x_back, y_end, z_center + half_length]
#             xml += f'        <geom name="tube{tube_id}" type="cylinder" fromto="{from_p[0]:.1f} {from_p[1]:.1f} {from_p[2]:.1f} {to_p[0]:.1f} {to_p[1]:.1f} {to_p[2]:.1f}" size="{tube_radius}" rgba="{rgba}" material="{material}"/>\n'
#             tube_id += 1
#
#         # Horizontal along x (depth) at y_start and y_end, each level
#         for z in horizontal_levels:
#             # At y_start
#             from_p = [x_front, y_start, z]
#             to_p = [x_back, y_start, z]
#             xml += f'        <geom name="tube{tube_id}" type="cylinder" fromto="{from_p[0]:.1f} {from_p[1]:.1f} {from_p[2]:.1f} {to_p[0]:.1f} {to_p[1]:.1f} {to_p[2]:.1f}" size="{tube_radius}" rgba="{rgba}" material="{material}"/>\n'
#             tube_id += 1
#             # At y_end
#             from_p = [x_front, y_end, z]
#             to_p = [x_back, y_end, z]
#             xml += f'        <geom name="tube{tube_id}" type="cylinder" fromto="{from_p[0]:.1f} {from_p[1]:.1f} {from_p[2]:.1f} {to_p[0]:.1f} {to_p[1]:.1f} {to_p[2]:.1f}" size="{tube_radius}" rgba="{rgba}" material="{material}"/>\n'
#             tube_id += 1
#
#         # Horizontal along y at x_front and x_back, each level
#         for z in horizontal_levels:
#             # Front
#             from_p = [x_front, y_start, z]
#             to_p = [x_front, y_end, z]
#             xml += f'        <geom name="tube{tube_id}" type="cylinder" fromto="{from_p[0]:.1f} {from_p[1]:.1f} {from_p[2]:.1f} {to_p[0]:.1f} {to_p[1]:.1f} {to_p[2]:.1f}" size="{tube_radius}" rgba="{rgba}" material="{material}"/>\n'
#             tube_id += 1
#             # Back
#             from_p = [x_back, y_start, z]
#             to_p = [x_back, y_end, z]
#             xml += f'        <geom name="tube{tube_id}" type="cylinder" fromto="{from_p[0]:.1f} {from_p[1]:.1f} {from_p[2]:.1f} {to_p[0]:.1f} {to_p[1]:.1f} {to_p[2]:.1f}" size="{tube_radius}" rgba="{rgba}" material="{material}"/>\n'
#             tube_id += 1
#
...
#                 # Front cross1: y_start low to y_end high
#                 from_p = [x_front, y_start, z_low]
#                 to_p = [x_front, y_end, z_high]
#                 xml += f'        <geom name="tube{tube_id}" type="cylinder" fromto="{from_p[0]:.1f} {from_p[1]:.1f} {from_p[2]:.1f} {to_p[0]:.1f} {to_p[1]:.1f} {to_p[2]:.1f}" size="{tube_radius}" rgba="{rgba}" material="{material}"/>\n'
#                 tube_id += 1
#                 # Front cross2: y_end low to y_start high
#                 from_p = [x_front, y_end, z_low]
#                 to_p = [x_front, y_start, z_high]
#                 xml += f'        <geom name="tube{tube_id}" type="cylinder" fromto="{from_p[0]:.1f} {from_p[1]:.1f} {from_p[2]:.1f} {to_p[0]:.1f} {to_p[1]:.1f} {to_p[2]:.1f}" size="{tube_radius}" rgba="{rgba}" material="{material}"/>\n'
#                 tube_id += 1
#                 # Back cross1
#                 from_p = [x_back, y_start, z_low]
#                 to_p = [x_back, y_end, z_high]
#                 xml += f'        <geom name="tube{tube_id}" type="cylinder" fromto="{from_p[0]:.1f} {from_p[1]:.1f} {from_p[2]:.1f} {to_p[0]:.1f} {to_p[1]:.1f} {to_p[2]:.1f}" size="{tube_radius}" rgba="{rgba}" material="{material}"/>\n'
#                 tube_id += 1
#                 # Back cross2
#                 from_p = [x_back, y_end, z_low]
#                 to_p = [x_back, y_start, z_high]
#                 xml += f'        <geom name="tube{tube_id}" type="cylinder" fromto="{from_p[0]:.1f} {from_p[1]:.1f} {from_p[2]:.1f} {to_p[0]:.1f} {to_p[1]:.1f} {to_p[2]:.1f}" size="{tube_radius}" rgba="{rgba}" material="{material}"/>\n'
#                 tube_id += 1
#
#     xml += '    </worldbody>\n'
